...
class cart
{
   ...
   {  
            ...
            try{
                        ...
                        if (v.passes()){
                            ...
                            let cartResult = await cartPageProductService.cartDataSave(cartData);                            
                            if (cartResult.errno) {
   ...
   {  
    try {
        ...
        if (v.passes()){
            ...
            let cartListResult = await cartPageProductService.cartDataList(req.body.userId); 
                               
            if (cartListResult.errno) {
                return res.status(500).json({ message: cartListResult.sqlMessage });
            } else {                      
                return res.status(200).json({ cartListResult, 'message': 'cart sucessfully save'})
            }
        }        
    } catch (error) {
        return res.status(400).json({'message': 'somthing went wrong'}); 
   ...
   async  cartDelete(req, res)
   {
    ...
    if (v.passes()){       
            try {                
                let cartlistRowDelete = await cartPageProductService.cartDelete(req.body.id); 
               //console.log(cartlistRowDelete);                  
                if(cartlistRowDelete.affectedRows)  
                {
